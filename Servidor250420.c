...
int ConectarMysql()
{
	...
	conn = mysql_real_connect (conn, "localhost","root", "mysql", NULL, 0, NULL, 0);  //shiva2.upc.es
	if (conn==NULL)
	{
...
{	
	...
	{		
		...
		if (codigo == 1 || codigo == 2 || codigo == 5 || codigo == 6 || codigo ==7 || codigo ==8)  //Saco el nombre			
		{			
			p = strtok( NULL, "/");			
			strcpy (nombre, p);			
			printf ("Nombre: %s\n", nombre);
		...
		else if (codigo == 7)  //manda la aceptacion
		{
			...
			printf("Notificaci\F3n: ", notificacion);
